...
const History: React.FunctionComponent<HistoryProps> = ({ history }: HistoryProps) => {
    ...
    return (
        <Container>
            <div style={{ borderBottom: '1px solid ' + pups.color('dark-verylighten'), padding: `${pups.ms(-1)} ${pups.ms(0)}` }}>History</div>
            ...
            <ul style={{ margin: 0, padding: `${pups.ms(-2)} 1px`, listStyle: 'none', maxHeight: '70vh', overflow: 'auto' }}>
                ...
                <HistoryItem current={!hasHistory} onClick={() => goToHistory(history.length)}>Original</HistoryItem>
            </ul>
        </Container>
    )
}
