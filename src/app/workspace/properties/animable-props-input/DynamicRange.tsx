...
import SimpleAnimationProp from 'app/workspace/properties/simple-animation/SimpleAnimation'
import Modal from 'components/Modal'
import { bValueAnimation, bValueTransformable, IProjectSceneChild, TAnimation } from 'urpflanze-ext'
...
const DynamicRange: React.FunctionComponent<IDynamicRangeProp> = ({
	...
	const bAnimation = bValueAnimation(value)
			...
			{bAnimation ? (
				<div className="dynamic-range__animation-value">
					{(value as TAnimation).type === 'simple' ? (
						<small>
							<i>
								{bValueTransformable(value.value.from) ? value.value.from.value : value.value.from} →
								{bValueTransformable(value.value.to) ? value.value.to.value : value.value.to} in{' '}
