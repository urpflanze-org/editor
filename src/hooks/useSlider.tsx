...
function useSlider<T extends HTMLElement>({ value, step, vertical, exp, min, max, onChange, events }: ISliderInteface): TSliderValueInteface<T>
{
    let initialValuePercentage = relativeClamp(value, min, max, 0, 100)

    const [currentValue, setCurrentValue] = React.useState({
        value, valuePercentage: initialValuePercentage
    })

    const sliderRef: React.RefObject<T> = useDraggable({
        onNotDrag: () => { events && events.notDrag && events.notDrag() },
        onDragStart: () => { events && events.dragStart && events.dragStart() },
        onDrag: (coords) => { 
    ...
    React.useEffect(() => {
        initialValuePercentage = relativeClamp(value, min, max, 0, 100)
