...
import { imageToCanvas } from 'app/workspace/drawer/DrawerUtilities'
...
const TemporanyImage: React.FunctionComponent<{ image: string; onSave: any; onRemove: any }> = ({
	image,
	onSave,
	onRemove,
}) => {
...
const DrawerBackgroundImage: React.FunctionComponent<{
	image: string
	onChange: (src: { image: CanvasImageSource; source: string } | string | null) => void
}> = ({ image, onChange }) => {
	...
	async function handleChange(fit: 'adapt' | 'fill' | 'none', opacity: number) {
		onChange(
			temporanyImage
				? await imageToCanvas(temporanyImage, temporanyImageSize, fit, opacity, temporanyImageQuality)
				: null
		)
		setBOpenModal(false)
	}
	...
	return (
		<React.Fragment>
			...
			<Modal open={bOpenModal} close={() => setBOpenModal(false)}>
				{temporanyImage ? (
					<TemporanyImage
						image={temporanyImage}
						onSave={async (fit, opacity) => handleChange(fit, opacity)}
