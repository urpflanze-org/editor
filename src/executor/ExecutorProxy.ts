...
import { Emitter } from '@urpflanze/drawer-canvas/dist/cjs'
	...
	public setDrawer(
		...
		if (!this.bCanvasMounted && canvas) {
			...
			if (this.bRunOnLocal) {
				...
				return this.proxyComunicationDispatcher('set-drawer', 'set-drawer', { canvas, size, resolution })
			} else {
				console.log('Drawer work on async service')
				const offscreen = canvas.transferControlToOffscreen()
				return this.proxyComunicationDispatcher('set-drawer', 'set-drawer', { canvas: offscreen, size, resolution }, [
					offscreen,
				])
			}
		} else {
			return this.proxyComunicationDispatcher('set-drawer', 'set-drawer', { size, resolution })
		}
	}
