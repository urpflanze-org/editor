...
const Color: React.FunctionComponent<ColorProps> = ({ value, enableAlpha, size, onChange }) => 
{
    ...
    React.useEffect(() => {
        ...
        function handleClick(e) 
        {
            if (
                !ctrlKeyPressed(e) && (
                    e.target == colorRef.current || colorRef.current?.contains(e.target)
                    || e.target == colorPickerRef.current || colorPickerRef.current?.contains(e.target)
                )
            )
    ...
    return (
        <React.Fragment>
            <div ref={colorRef} style={{ display: 'flex', alignItems: 'center' }}>
